...
{
   private MaskedText mask;

  /**
   * Constructs the controller with the specified masked text.
   * @param <code>m</code> the specified masked text.
   */
   public MaskPosController(MaskedText m) {
     mask = m;
   }

   public /*C#override*/ void seek (int v) {
     if (v != 0) setOffset(mask.findHandledTag(getOffset() + v, v>0?1:-1));
   }
}
